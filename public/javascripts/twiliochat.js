...
const twiliochat = (function() {
    ...
    const notifyTyping = $.throttle(function() {
    ...
    function fetchAccessToken(username, handler) {
      $.post('/twilio', {identity: username, device: 'browser'}, null, 'json')
    ...
    tc.addMessageToList = function(message) {
      const rowDiv = $('<div>').addClass('row no-margin');
    ...
    function updateChannelUI(selectedChannel) {
      const channelElements = $('.channel-element').toArray();
      const channelElement = channelElements.filter(function(element) {
    ...
    function addChannel(channel) {
      ...
      const rowDiv = $('<div>').addClass('row channel-row');
      rowDiv.loadTemplate('#channel-template', {
        channelName: channel.friendlyName
      });
  
      const channelP = rowDiv.children().children().first();
    ...
    function selectChannel(event) {
      ...
      const channelSid = target.data().sid;
      const selectedChannel = tc.channelArray.filter(function(channel) {
